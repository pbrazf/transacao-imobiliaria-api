...
class ComissaoSaida(BaseModel):
    ...
    id: UUID = Field(
        description='Identificador único (UUID) da comissão'
    )
    transacao_id: UUID = Field(
        description='Identificador único (UUID) da transação associada'
	)
    percentual: Decimal = Field(
    ...
    paga: bool = Field(
        description='Indica se a comissão já foi paga (true/false)'
    )
